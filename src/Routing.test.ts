import { urlToGraphQueryPage, urlToObjectFactQueryPage, urlToObjectSummaryPage } from './Routing';
...
it('can make url to the object fact query page', () => {
  expect(
    urlToObjectFactQueryPage({ objectTypeName: 'uri', objectValue: 'http://this.is.a.test.com/?x=abc&y=123' })
  ).toEqual('/object-fact-query/uri/http%3A%2F%2Fthis.is.a.test.com%2F%3Fx%3Dabc%26y%3D123');
});

it('can make url to the graph query page', () => {
  expect(
    urlToGraphQueryPage({
      objectTypeName: 'threatActor',
      objectValue: 'Sofacy',
      query:
