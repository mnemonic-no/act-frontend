...
const ObjectComp = ({title, actObject}: {
    title: string,
    actObject: ActObject
}) => {
...
const ObjectSelectionComp = observer(({title, selectionObject, validObjectTypes, onChange}: {
    title: string,
...
const RoundedBox = ({children, style, title}: any) => {
    return (
        <div>
            {title && (<Typography align='center' color='primary' variant='subtitle1'>{title}</Typography>)}
            <div style={{
...
const OneLeggedFact = observer(({store}: { store: DialogStore }) => {
    return (
        <div style={{
            ...
            <div style={{flex: "1"}}><ObjectComp actObject={store.selectedObject} title='Source'/></div>
...
const UniDirectionalFact = observer(({store, form, onChange, onSwapClick}: {
    store: DialogStore,
    ...
    const SourceComp = observer(() => {
        ...
        if (form.isSelectionSource) {
            ...
            return (<ObjectSelectionComp title='Source'
    ...
    const DestinationComp = observer(() => {
        if (!form.isSelectionSource) {
            ...
            return (<ObjectSelectionComp title='Destination'
                                         selectionObject={form.otherObject}
                                         validObjectTypes={form.validOtherObjectTypes}
                                         onChange={onChange}/>);
        }
        ...
        <Dialog
            ...
            <form
                ...
                <DialogContent classes={{root: classes.dialogContentRoot}}>
                    ...
                    <div style={{marginTop: "20px"}}>
                        <AccessModeSelector
                            value={store.accessMode}
                            onChange={(v: any) => store.onAccessModeSelectorChange(v)}/>
                    </div>
