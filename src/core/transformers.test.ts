...
it('can determine if fact type is one-legged', () => {
  ...
  expect(
    isOneLeggedFactType(factType({ relevantObjectBindings: [{ sourceObjectType: { id: '1', name: 'something' } }] }))
  ).toBeTruthy();
  expect(
    isOneLeggedFactType(
...
it('can make object label', () => {
  ...
  expect(objectLabel(theObject, 'name', facts)).toBe('Some name');

  expect(objectLabel(theObject, 'name', [])).toBe('may be overridden');
});
...
it('test valid unidirectional fact target object types', () => {
  ...
  expect(validUnidirectionalFactTargetObjectTypes(attributedTo, actObject({ type: threatActor }), false)).toEqual([
    incident
  ]);

  expect(validUnidirectionalFactTargetObjectTypes(attributedTo, actObject({ type: threatActor }), true)).toEqual([]);
});

it('one legged facts for', () => {
  const matchingFact = fact({ id: 'f1', sourceObject: { id: '1' }, destinationObject: undefined });

  expect(oneLeggedFactsFor(actObject({ id: '1' }), [matchingFact, fact({ id: 'f2' })])).toEqual([matchingFact]);
});
