...
class RefineryStore {
    ...
    constructor(root: MainPageStore) {
        ...
        reaction(() => this.root.queryHistory.result.objects,
            (resultObjects) => {
    ...
    static refineResult(queryResult: QueryResult, objectTypeFilters: Array<ObjectTypeFilter>, endTimestamp: Date | string, showRetractions: boolean): QueryResult {
        ...
        let filteredFacts: { [id: string]: ActFact } = R.filter((val: ActFact) => {
            return factsToObjects([val]).every(
                (object: any) => !excludedObjectTypeIds.includes(object.type.id)
            )
        }, queryResult.facts);
        ...
        // Retractions
        const retractions: Array<ActFact> = Object.values(filteredFacts).filter((fact: ActFact) => fact.type.name === 'Retraction');
        ...
        const retractedFacts = R.pipe(
            ...
            R.map((fact: ActFact) => ({
                    ...
                    retraction: retractions.find(
                        (retraction: any) => fact.id === retraction.inReferenceTo.id)
                })
            ))(filteredFacts);
